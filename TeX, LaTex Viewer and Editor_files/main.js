...
function init(){
	...
	_getid("o_nonstopmode").onclick=function(){
		localStorage["o_nonstopmode"]=this.checked;
	}		
	...
	document.documentElement.onmouseup=function(e){
		...
		proc_closesize();
		if(!u_uploading) proc_closeoption();
		//proc_closeoutput();
	}
}
var glastblob;
function handleFileSelect(files){
	...
	function go(idx){
		...
		var ispdf=false;
		if(getfileext(f.name || '')=='pdf'){
			fmaxsize=150;
			ispdf=true;
		}
		if(f.size>fmaxsize*1024*1024){
			alert('The file size is too large to edit. (around '+fmaxsize+' MB limit)');
			return;
		}
		if(ispdf){
		...
		_blobtotext(f,'UTF-8',function(s){
			var resp={title:f.name};
			proc_newtab(s,resp);
...
function proc_setoption(){
	...
	//var a=localStorage['o_nonstopmode'] || 'true';
	//if(a) _getid('o_nonstopmode').checked=toBool(a);
	_getid('o_nonstopmode').checked=true;
}
...
function showhotkeyhelp(){
	var s='F9 or Ctrl+Enter: View as PDF';
	s=s+'\n\nCtrl+Space: LaTeX Command Autocomplete (blank or \\ or some letters) and Ctrl+Space';
	s=s+'\n\nCtrl+G : Jump to line';
	s=s+'\n\nCtrl+F : Find';
...
var gsyntaxs2={"gfm":["xml","markdown"],"htmlmixed":["xml","javascript","css"],"markdown":["xml"],"php":["xml","javascript","css","clike"]};
...
function proc_onresize(){
	...
	var h=wh-c_pdfviewer.offsetHeight-12;
